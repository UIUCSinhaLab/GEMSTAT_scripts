...
class GEMSTAT_Motif(Bio.motifs.Motif):
    ...
    def __init__(self, name, counts, pseudocount=0.0, comments=None, alphabet="ACGT"):
        ...
        M = counts_np.shape[1]
        if M != len(alphabet):
            raise Exception("Alphabet size and count size do not agree")
        
        counts_as_biopython_wants = dict([(alphabet[i],counts_np[:,i]) for i in range(M)])
        
        super(GEMSTAT_Motif,self).__init__(counts = counts_as_biopython_wants)
        ...
        #This is absurdly general for something that will only be used with GEMSTAT.
        if isinstance(alphabet, Bio.Alphabet.Alphabet):
            self.alphabet = alphabet
        elif isinstance(alphabet, str):
            self.alphabet = Bio.Alphabet.DNAAlphabet()
            self.alphabet.letters = alphabet
